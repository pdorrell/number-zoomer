...
export class AppStore implements ZoomableInterface {
  ...
  get canvasEquationGraph(): CanvasEquationGraph {
    ...
    const screenPoints = equationPoints.map(point => {
      const screenCoord = this.mapping.worldToScreen(point);
      return new ScreenPoint(screenCoord.x, screenCoord.y);
    });
    const worldPoints = equationPoints.map(point => [point[0].toNumber(), point[1].toNumber()] as [number, number]);
    const shouldDrawAsCurve = this.equation.shouldDrawAsCurve(extendedWorldWindow);
    return new CanvasEquationGraph(screenPoints, worldPoints, shouldDrawAsCurve);
  }
  ...
  private initializeWorldWindowWithAspectRatio() {
    ...
    // Update coordinate mapping with new world window
    this.mapping = new CoordinateMapping(this.screenViewport, this.worldWindow, this.extension);
  }
  ...
  resetView() {
    // Use the same initialization logic to maintain 1:1 aspect ratio
    this.initializeWorldWindowWithAspectRatio();
